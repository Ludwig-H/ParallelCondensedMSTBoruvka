...
def boruvka_mst(np.ndarray[INT_t, ndim=1] U_arr,
                ...
                int n_threads=0):
    """
    Simple robust MST (Boruvka-style pass) without OpenMP to guarantee portability.
    Returns (Uo, Vo, Wo, Ei) where Ei are original edge indices.
    """
    ...
    while edges_added < n - 1:
        for i in range(n):
            bestW[i] = BIG; bestU[i] = -1; bestV[i] = -1; bestE[i] = -1

        # scan edges
        for i in range(m):
            u = U[i]; v = V[i]; w = W[i]
            cu = uf_find(parent, u); cv = uf_find(parent, v)
            if cu == cv: continue
            if w < bestW[cu]:
                bestW[cu] = w; bestU[cu] = u; bestV[cu] = v; bestE[cu] = i
            if w < bestW[cv]:
                bestW[cv] = w; bestU[cv] = u; bestV[cv] = v; bestE[cv] = i

        # add chosen edges
        cdef int updated = 0
        for aidx in range(n):
            if bestU[aidx] == -1: continue
            u = bestU[aidx]; v = bestV[aidx]; w = bestW[aidx]; eidx = bestE[aidx]
            cu = uf_find(parent, u); cv = uf_find(parent, v)
            if cu == cv: continue
            uf_union(parent, rank, cu, cv)
            Uo[edges_added] = u; Vo[edges_added] = v; Wo[edges_added] = w; Ei[edges_added] = eidx
            edges_added += 1; updated += 1
